...
但是问题是，如果要给`<bbb>`组件中加原生事件，这个可以使用`native`修饰符,这个在模板解析的时候会被解析成`data.nativeOn`,这个属性就会被绑定在模板中的根原生html元素上，通常来说这样的需求很少，一般自定义组件通常都是通过发订阅者模式来触发事件，也就是把所有的`@`修饰的事件以`listener`形式对来,通过`emit`来触发。就像上面一样。
